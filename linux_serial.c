...
int main(){
    ...
    tty.c_cflag &= ~CSTOPB; // Clear stop field, only one stop bit used in communicacation.
    ...
    unsigned char message[] = {'i', 'l', 'h', 'a', 'n'};
    ...
    // call printf() easily.
    memset(&read_buffer, '\0', sizeoff(read_buffer));

    // read bytes. The behaviour pf read()
    ...
    // n is the number of bytes read, n may be 0 if no bytes were received, and can also be -1 to signal error.
    if (num_bytes < 0)  
    {
